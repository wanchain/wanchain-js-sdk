...
let logger = utils.getLogger('StoremanStakeClaimDataCreator.js');

/**
 * @class
 * @augments TxDataCreator
 */
class StoremanStakeClaimDataCreator extends TxDataCreator {
    /**
     * @constructor
    ...
    async createCommonData(){
        logger.debug("Entering StoremanStakeClaimDataCreator::createCommonData");
        ...
        try{
            ...
            if(this.config.useLocalNode === true){
                ...
                logger.info("StoremanStakeClaimDataCreator::createCommonData getNonceByWeb3,%s",commonData.nonce);
            }else{
                commonData.nonce  = await ccUtil.getNonceByLocal(commonData.from, this.input.chainType);
                logger.info("StoremanStakeClaimDataCreator::createCommonData getNonceByLocal,%s",commonData.nonce);
            }
    ...
    createContractData(){
        try{
            ...
            logger.error("StoremanStakeClaimDataCreator::createContractData: error: ",error);
            this.retResult.result = error;
            this.retResult.code   = false;
        }
...
module.exports = StoremanStakeClaimDataCreator;
