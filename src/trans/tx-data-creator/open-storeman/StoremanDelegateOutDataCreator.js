...
let logger = utils.getLogger('StoremanDelegateOutDataCreator.js');

/**
 * @class
 * @augments TxDataCreator
 */
class StoremanDelegateOutDataCreator extends TxDataCreator {
    /**
     * @constructor
    ...
    async createCommonData(){
        logger.debug("Entering StoremanDelegateOutDataCreator::createCommonData");
        ...
        try{
            ...
            if(this.config.useLocalNode === true){
                ...
                logger.info("StoremanDelegateOutDataCreator::createCommonData getNonceByWeb3,%s",commonData.nonce);
            }else{
                commonData.nonce  = await ccUtil.getNonceByLocal(commonData.from, this.input.chainType);
                logger.info("StoremanDelegateOutDataCreator::createCommonData getNonceByLocal,%s",commonData.nonce);
            }
    ...
    createContractData(){
        try{
            ...
            logger.error("StoremanDelegateOutDataCreator::createContractData: error: ",error);
            this.retResult.result = error;
            this.retResult.code   = false;
        }
...
module.exports = StoremanDelegateOutDataCreator;
