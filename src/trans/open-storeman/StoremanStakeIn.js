...
let StoremanStakeInDataCreator = require('../tx-data-creator/open-storeman/StoremanStakeInDataCreator');

let ccUtil = require('../../api/ccUtil');
let error  = require('../../api/error');
let utils  = require('../../util/util');

let logger = utils.getLogger('StoremanStakeIn.js');

/**
 * @class
 * @augments StoremanStakeIn
 */

 class StoremanStakeIn extends NormalChain {
       /**
     * @constructor
  ...
  createDataCreator(){
      logger.debug("Entering StoremanStakeIn::createDataCreator");
      this.retResult.code = true;
      this.retResult.result = new StoremanStakeInDataCreator(this.input, this.config);
      logger.debug("StoremanStakeIn::createDataCreator is completed.");
      return this.retResult;
  }

  createDataSign(){
      logger.debug("Entering StoremanStakeIn::createDataSign");
      ...
      logger.debug("StoremanStakeIn::createDataSign is completed, result=", this.retResult.code);
      return this.retResult;
  }
  ...
  transFailed(){
      ...
      logger.error("StoremanStakeIn::transFailed");
  ...
  postSendTrans(resultSendTrans){
      logger.debug("Entering StoremanStakeIn::postSendTrans");
      ...
      record.sentTime = cur;
      logger.debug("StoremanStakeIn::postSendTrans");
      logger.debug("record is :",ccUtil.hiddenProperties(record,['x']));
      global.wanDb.updateItem(this.config.normalCollection,{hashX:record.hashX},record);

      this.retResult.code = true;
      logger.debug("StoremanStakeIn::postSendTrans is completed.");
      return this.retResult;
  }
 }

 module.exports = StoremanStakeIn;
