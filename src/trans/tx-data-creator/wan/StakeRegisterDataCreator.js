...
let logger = utils.getLogger('StakeRegisterDataCreator.js');

/**
 * @class
 * @augments TxDataCreator
 */
class StakeRegisterDataCreator extends TxDataCreator {
    /**
     * @constructor
    ...
    async createCommonData(){
        logger.debug("Entering StakeRegisterDataCreator::createCommonData");
        ...
        try{
            ...
            if(this.config.useLocalNode === true){
                ...
                logger.info("StakeRegisterDataCreator::createCommonData getNonceByWeb3,%s",commonData.nonce);
            }else{
                commonData.nonce  = await ccUtil.getNonceByLocal(commonData.from, this.input.chainType);
                logger.info("StakeRegisterDataCreator::createCommonData getNonceByLocal,%s",commonData.nonce);
            }
    ...
    createContractData(){
        try{
            logger.debug("Entering StakeRegisterDataCreator::createContractData");
            ...
            let data = ccUtil.getDataByFuncInterface(this.contract.ABI,
                ...
                this.input.secPk,
                this.input.bn256Pk,
                this.input.lockEpoch,
                this.input.feeRate,
                this.input.maxFeeRate);

            this.retResult.result = data;
            this.retResult.code   = true;
        } catch(error) {
            logger.error("StakeRegisterDataCreator::createContractData: error: ",error);
            this.retResult.result = error;
            this.retResult.code   = false;
        }
...
module.exports = StakeRegisterDataCreator;
