...
let logger = utils.getLogger('StoremanStakeInDataCreator.js');

/**
 * @class
 * @augments TxDataCreator
 */
class StoremanStakeInDataCreator extends TxDataCreator {
    /**
     * @constructor
    ...
    async createCommonData(){
        logger.debug("Entering StoremanStakeInDataCreator::createCommonData");
        ...
        try{
            ...
            if(this.config.useLocalNode === true){
                ...
                logger.info("StoremanStakeInDataCreator::createCommonData getNonceByWeb3,%s",commonData.nonce);
            }else{
                commonData.nonce  = await ccUtil.getNonceByLocal(commonData.from, this.input.chainType);
                logger.info("StoremanStakeInDataCreator::createCommonData getNonceByLocal,%s",commonData.nonce);
            }
    ...
    createContractData(){
        try{
            logger.debug("Entering StoremanStakeInDataCreator::createContractData");
            ...
            logger.error("StoremanStakeInDataCreator::createContractData: error: ",error);
            this.retResult.result = error;
            this.retResult.code   = false;
        }
...
module.exports = StoremanStakeInDataCreator;
